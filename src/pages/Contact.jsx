...
const Contact = () => {
    ...
    useEffect(() => emailjs.init(env.EMAILJS_PUBLIC_API_KEY), []);
    ...
    */
    async function handleContact() {
        const apiURL = 'https://emailvalidation.abstractapi.com/v1/?api_key=' + env.ABSTRACT_EMAIL_API_KEY
        ...
        if(name.current.value !== "" && msg.current.value !== ""){
            if(emailItem){
                try {
                    ...
                    if (isValidSMTP) {
                        await emailjs.send(env.EMAILJS_SERVICE_ID, env.EMAILJS_TEMPLATE_ID, params).then((res) => {
